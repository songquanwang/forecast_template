...
def get_urt(address):
    """
    获取地址经纬度
    {"status":0,"result":{"location":{"lng":116.21412109279787,"lat":39.94960819303885},
    "precise":1,"confidence":80,"comprehension":44,"level":"生活服务"}}
    :param address:
    :return:
    """
    ak = 'hPOW2xN1TzUTKQUqaLFiehUV0g2ypc36'
    queryStr = '/geocoder/v2/?address=%s&output=json&ak={}'.format(ak, address)
    # 对queryStr进行转码，safe内的保留字符不转换
    encodedStr = parse.quote(queryStr, safe="/:=&?#+!$,;'@()*[]")
    # 在最后直接追加上yoursk
    rawStr = encodedStr + 'hPOW2xN1TzUTKQUqaLFiehUV0g2ypc36'
    # 计算sn
    ...
    response = urllib.request.urlopen(url)
    content = response.read().decode('utf-8')
    result = eval(content)['result'].location
