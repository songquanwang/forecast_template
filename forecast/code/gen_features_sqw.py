...
def get_plan_df(data):
    ...
    plans_df.loc[(plans_df['transport_mode'] == 4) & (plans_df['distance'] < 100), 'dj1'] = plans_df.loc[(plans_df['transport_mode'] == 4) & (plans_df['distance'] < 100), 'mdj']
...
def gen_plan_feas(plans_df):
    ...
    plans_df['mode_texts'] = plans_df.groupby('sid')['transport_mode'].transform(lambda x: ' '.join(['word_{}'.format(mode) for mode in x.values]))
...
def get_train_test_feas_data():
    ...
    feature_columns = ['sid', 'pid', 'click_mode','o1','o2','d1','d2',
                       'eta', 'transport_mode', 'dj1', 'distance1', 'price1', 'first_mode', 'max_dist', 'min_dist', 'mean_dist', 'std_dist', 'max_price', 'min_price',
